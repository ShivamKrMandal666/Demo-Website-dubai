STEP 1 — NAVBAR ORDER
Update the navbar so the page order is: Home, Treatments, Doctors, Contact, Gallery (previously Home, Treatments, Doctors, Gallery, Contact). Just reorder the existing nav links/routes — don't change styling or add anything new.

STEP 2 — GALLERY PAGE
Build a new Gallery page that is intentionally isolated from the rest of the site's design system.

RULES — READ CAREFULLY
- Do NOT include the Navbar, Google Map, hero section, or Footer on this page. This page has none of the layout other pages share.
- Add only a leave/back button (fixed position, always accessible) so a visitor can exit back to the site (e.g. back to Home).
- Do NOT use the site's existing reusable components or CSS variables (colors, spacing, typography tokens, card components, buttons, etc.) on this page — build everything for this page standalone/scoped so it looks and feels deliberately different from the rest of the site.
- Do NOT extract or promote anything you build on this page into the shared components folder or shared CSS variables — nothing from this page should be reused elsewhere on the site. Keep all styles and components local/scoped to the gallery route only.

GALLERY COMPONENT
I'm using a 21st.dev infinite drag + scroll gallery component. Here's the component prompt/code to implement it:
...
IMAGES
- Populate the gallery with every image currently in the codebase — treatment images, doctor images, hero/background images, any other images used anywhere on the site — all of them should show up in this gallery.
- Lazy load all gallery images — nothing should load until the user actually navigates to the Gallery page (no preloading/prefetching this page's image set from other routes).
- Optimize every image used in the gallery for fast load, resizing as appropriate for how it displays in the grid/drag view.

PERFORMANCE
- This page should only load its assets (images, gallery component JS) when visited — no eager loading from other pages.
- Keep the page fast despite the image volume: lazy load, optimize, and avoid layout shift as images load in.

Confirm the full list of images you found in the codebase before finalizing the gallery, so I can check nothing's missing and nothing unwanted got included.